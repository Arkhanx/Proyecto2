...
for (var i = 0; i <= 3; i++) {
  ...
  // Aqui utilizamos la variable "i" del for para muchas cosas, la primera es para indicarle que debe escribir en todos los ids "valor#", utilizo el "i+1" debido a que no tengo ningun id en el html de valor que comience por 0.
   // Luego utilizamos nuevamente la variable "i" para acceder al inidice del array "respuesta".
  $('#valor'+(i+1)).val(respuesta[i].slice(0, -1) + valorTolerancia[respuesta[i][respuesta[i].length-1]]);
